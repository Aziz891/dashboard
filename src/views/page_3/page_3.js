...
class page_3 extends page_1 {
  ...
  handleDateChange_table = (date,  element) => {
    ...
    axios.get(`http://127.0.0.1:8000/faults/cape/?data=7&fromdate=${this.state.startDate.toISOString().slice(0, 10)}&todate=${this.state.endDate.toISOString().slice(0, 10)}`
    ).then(response => {
  ...
  render() {
    ...
    <div>
      ...
      <br/>
          ...
          <Thetable data={this.state.tableData} columns= {columns} title={"Bus Entries in CAPE"} />
      ...
      return (
        ...
        <div>
          ...
          <div>
            ...
            <Dashboard url_request={this.line_url}  url={'ped'} title={'Bus Entries'} startDate={this.state.startDate} endDate={this.state.endDate} ref={this.lineElement} />
          </div>
          <div>
